...
//        }
        ...
        BasePagerAdapter calendarAdapter = getPagerAdapter(mContext, this,flag);
        ...
        post(new Runnable() {
            ...
            public void run() {
                ...
                if (mOnCalendarChangedListener != null && !(mSelectedModel == SelectedModel.MULTIPLE) && getVisibility() == VISIBLE) {
                    //单选
                    mOnCalendarChangedListener.onCalendarChange(BaseCalendar.this, yearMonthLocalDate.getYear(), yearMonthLocalDate.getMonthOfYear(),
                            currentSelectDateList.size() == 0 ? null : currentSelectDateList.get(0),isTouch);
                    isTouch = false;
                }
...
//        notifyCalendar();
    ...
    //回去viewpager的adapter
    protected abstract BasePagerAdapter getPagerAdapter(Context context, BaseCalendar baseCalendar,int flag);
