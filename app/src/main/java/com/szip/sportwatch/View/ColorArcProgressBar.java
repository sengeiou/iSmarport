...
//        curSpeedPaint.setTextSize(curSpeedSize);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        {
            //画刻度线
            for(int i = 0; i < 90; i++)//把整个圆划分成90小份
            {
                if(i > 33 && i < 57)//33~57小份不需要显示
                {
                    canvas.rotate(4, centerX, centerY);//90小份,每份4°
                    continue;
                }
                ...
                canvas.drawLine(centerX, centerY - diameter / 2 + progressWidth / 2 + DEGREE_PROGRESS_DISTANCE
                        + (longDegree - shortDegree) / 2, centerX, centerY - diameter / 2 + progressWidth / 2 +
                        DEGREE_PROGRESS_DISTANCE + (longDegree - shortDegree) / 2 - shortDegree, degreePaint);
